...
export async function associateFilesWithNote(
  ...
  try {
    ...
    // Update files to associate with note
    await db
      .update(uploadFiles)
      .set({ noteId })
      .where(inArray(uploadFiles.id, fileIds));
    ...
    console.error("Failed to associate files with note:", error);
    return {
      success: false,
...
/**
 * Disassociate files from a note (set noteId to null)
 */
export async function disassociateFilesFromNote(
  fileIds: number[]
): Promise<{ success: boolean; error?: string }> {
  if (fileIds.length === 0) {
    return { success: true };
  ...
  try {
    ...
    console.error("Failed to disassociate files from note:", error);
    return {
      success: false,
...
export async function deleteFilesForNote(
  ...
  try {
    ...
    const files = await getFilesByNoteId(noteId);
    ...
    console.error("Failed to delete files for note:", error);
    return {
      success: false,
