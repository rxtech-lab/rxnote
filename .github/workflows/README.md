...
Runs end-to-end tests for the backend web application.
...
1. **Encode your Secrets.xcconfig file:**
   ...
   base64 -i RxNote/RxNote/Config/Secrets.xcconfig | pbcopy
   ```
...
- name: Setup Secrets.xcconfig from GitHub Secrets
  ...
  run: |
    echo "$SECRETS_XCCONFIG_BASE64" | base64 --decode > RxNote/RxNote/Config/Secrets.xcconfig
```
...
cp RxNote/RxNote/Config/Secrets.xcconfig.example RxNote/RxNote/Config/Secrets.xcconfig
...
SCHEME=RxNoteClips ./scripts/ios-build.sh
```
