...
3. **Decode secrets** - Writes `Secrets.xcconfig`, `RxNote/.env`, and `admin/.env` from base64-encoded GitHub Secrets
...
**File:** `RxNote/ci_scripts/ci_post_clone.sh`
...
if [ -n "$CI_TAG" ]; then
    ...
    sed -i '' "s/MARKETING_VERSION = .*;/MARKETING_VERSION = $VERSION;/g" \
        RxNote.xcodeproj/project.pbxproj
...
| `RXNOTE_TESTING_SECRET` | Base64-encoded `.env` with test credentials |
...
| `RXNOTE_TESTING_SECRET` | GitHub Secret | Environment Variable | Test credentials for E2E tests |
...
| `RxNote/ci_scripts/ci_post_clone.sh` | Xcode Cloud post-clone script - version bumping and environment setup |
