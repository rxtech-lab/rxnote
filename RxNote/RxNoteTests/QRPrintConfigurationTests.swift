...
struct QRPrintConfigurationTests {
    ...
    func defaultConfiguration() {
        ...
        #expect(config.fieldOrder.count == 2)
        #expect(config.enabledFields == [.title])
        #expect(!config.enabledFields.contains(.note))
    ...
    func activeFieldsFiltering() {
        ...
        // Default: title enabled
        let active = config.activeFields
        #expect(active.count == 1)
        #expect(active[0] == .title)
        #expect(!active.contains(.note))
    }

    @Test("Active fields reflects both toggle and reorder changes")
    func activeFieldsAfterToggleAndReorder() {
        let config = QRPrintConfiguration(
            enabledFields: [.title, .note]
        )

        // Move note before title
        config.moveFields(from: IndexSet(integer: 1), to: 0)

        let active = config.activeFields
        #expect(active.count == 2)
        #expect(active[0] == .note)
