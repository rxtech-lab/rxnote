...
final class QRPrintConfiguration {
    ...
    init(
        ...
        enabledFields: Set<QRPrintField> = [.title],
    ...
    static func loadSaved() -> QRPrintConfiguration {
        ...
        fieldOrder.removeAll { field in
            !QRPrintField.allCases.contains(field)
        }
        ...
        return QRPrintConfiguration(
            fieldOrder: fieldOrder,
            enabledFields: Set(stored.enabledFields.filter { QRPrintField.allCases.contains($0) }),
    ...
    /// Extract the string value for a field from note title and content
    func fieldValue(for field: QRPrintField, title: String, note: String?) -> String? {
        switch field {
        case .title:
            return title
        case .note:
